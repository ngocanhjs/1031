...
fig_box = px.box(
    data, x="MAIN_GENRE", y="SCORE", color="MAIN_GENRE", title="The box chart demonstrates the distribution of range score of TV shows according to TV show genres",
...
def update_bar_chart(value):
    ...
    fig_bar.update_layout(title=f'Top {value} countries that have the most TV shows in the period 1970 - 2020')
...
def update_box_chart(genre_selection):
    ...
    fig = px.box(
        data_subset, x="MAIN_GENRE", y="SCORE", color="MAIN_GENRE", title=f"The chart for {genre_selection} genre",
        color_discrete_map={genre: color for genre, color in zip(data['MAIN_GENRE'].unique(), ['goldenrod','hotpink','chocolate','lawngreen','dodgerblue','darkviolet','plum','forestgreen','crimson','yellow'])}
    )
    return fig
...
def update_scatter_chart(genre_selection, year_range):
    data_subset = data.loc[
        (data['MAIN_GENRE'].isin(genre_selection)) & (data['RELEASE_YEAR'].isin(range(year_range[0], year_range[1]+1)))
    ]
    fig = px.scatter(
